  ...
  overlay.innerHTML = `
    <div class="pag-detail-panel card">
      ...
      <div class="pag-detail-body">
        ...
        ${pagField('$','Valor Financiado',formatCurrency(venda.valorFinanciado || venda.valor))}${pagField('$','Valor Entrada',formatCurrency(venda.valorEntrada || venda.entrada))}
        ${pagField('T','Tipo Entrada',venda.tipoEntrada)}${pagField('T','Banco Financiamento',venda.bancoFinanciamento)}
        ...
        ${venda.camposExtras && Object.keys(venda.camposExtras).length ? Object.entries(venda.camposExtras).map(([k, v]) => pagField('＋', k, esc(v))).join('') : ''}
      </div>
      ${editable ? `
  ...
  PAGAMENTOS_CONFIG.STATUS.forEach(st => {
    ...
    html += `<div class="pag-status-group"><div class="pag-group-head">${pagStatusBadge(st.id)}<span class="chip gold">${items.length}</span></div>
      <div class="tbl-wrap"><table class="pag-table"><thead><tr>
        ...
        <td>${esc(v.cpf)}</td><td class="num"><b>${formatCurrency(v.valorFinanciado || v.valor)}</b></td>
