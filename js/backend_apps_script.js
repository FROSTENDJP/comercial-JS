...
const SHEET_FATURAMENTO = 'BD_Faturamento';
const SHEET_FORM_SCHEMA = 'FormSchema';

// Schema inicial do formulário de Vendas — reproduz exatamente os campos que
// já existiam fixos no vendas.html. "core": true identifica os campos que o
// sistema usa em KPIs/relatórios e por isso não podem ser removidos nem
// trocar de tipo pelo editor da Gerência (só rótulo, obrigatoriedade e ordem).
const DEFAULT_VENDAS_SCHEMA = [
  { id: 'cliente', label: 'Cliente', type: 'text', required: true, core: true, order: 1 },
  { id: 'cpf', label: 'CPF', type: 'text', required: false, core: true, order: 2 },
  { id: 'modelo', label: 'Modelo', type: 'select', options: ['AZ1', 'AZ125', 'AZ160'], required: true, core: true, order: 3 },
  { id: 'valorFinanciado', label: 'Valor financiado', type: 'money', required: true, core: true, order: 4 },
  { id: 'valorEntrada', label: 'Valor entrada', type: 'money', required: true, core: true, order: 5 },
  { id: 'formaPagamento', label: 'Forma da entrada', type: 'text', required: true, core: true, order: 6 },
  { id: 'bancoFinanciamento', label: 'Banco financiamento', type: 'text', required: false, core: true, order: 7 },
  { id: 'vendedor', label: 'Vendedor', type: 'colaborador', required: true, core: true, order: 8 },
  { id: 'cidade', label: 'Cidade', type: 'text', required: false, core: true, order: 9 },
  { id: 'chassi', label: 'Chassi', type: 'text', required: false, core: true, order: 10 }
];

// Deve ficar em sincronia com PAGAMENTOS_CONFIG.UNIDADES (js/pagamentos-config.js).
// Usada para validar a unidade informada ao cadastrar um colaborador.
const UNIDADES_VALIDAS = [
  'mamanguape', 'guarabira', 'patos-centro', 'patos-cd', 'natal',
  'joao-pessoa', 'parnamirim', 'parnamirim-matriz-rn', 'sape', 'pombal', 'sao-bento', 'beira-rio'
];
...
function doPost(e) {
  try {
    ...
    // Perfil Vendedor/Loja pode pedir também EXCLUSÃO de um lançamento já
    // feito (não só edição). Perfil Comercial (GERENCIA) aprova ou rejeita
    // qualquer um dos dois tipos de solicitação.
    if (action === 'vendasRequestExclusao') {
      return responseJson({ success: true, data: requestVendaExclusao(params) });
    }

    if (action === 'vendasApproveExclusao') {
      return responseJson({ success: true, data: approveVendaExclusao(params) });
    }

    if (action === 'vendasRejectSolicitacao') {
      return responseJson({ success: true, data: rejectVendaSolicitacao(params) });
    }

    // Faturamento (revendedores em lote) — restrito ao perfil FATURAMENTO
    // para criar; Financeiro confirma o pagamento separadamente das vendas
    // de loja.
    if (action === 'faturamentoList') {
      return responseJson({ success: true, data: listFaturamentosScoped(params) });
    }

    if (action === 'faturamentoCreate') {
      return responseJson({ success: true, data: createFaturamento(params) });
    }

    if (action === 'faturamentoConfirmar') {
      return responseJson({ success: true, data: confirmFaturamentoPagamento(params) });
    }

    // Editor de formulário dinâmico (Prompt 1) — leitura liberada para quem
    // usa o formulário (UNIDADE) e para quem edita (GERENCIA); gravação
    // restrita à Gerência (perfil Comercial).
    if (action === 'formSchemaGet') {
      requireVendasUser(params, ['UNIDADE', 'GERENCIA']);
      return responseJson({ success: true, data: getFormSchema(String(params.formularioId || 'vendas').trim()) });
    }

    if (action === 'formSchemaSave') {
      return responseJson({ success: true, data: saveFormSchema(params) });
    }

    // Gestão de Colaboradores (perfil GERENCIA do setor Vendas).
    // colaboradoresList é usada tanto pela Gerência (gestão) quanto pela
    // própria loja (perfil UNIDADE) para popular o dropdown de vendedor —
    // veja listColaboradoresScoped() para o escopo aplicado a cada perfil.
    if (action === 'colaboradoresList') {
      return responseJson({ success: true, data: listColaboradoresScoped(params) });
    }

    if (action === 'colaboradoresCreate') {
      return responseJson({ success: true, data: createColaborador(params) });
    }

    if (action === 'colaboradoresSetStatus') {
      return responseJson({ success: true, data: setColaboradorStatus(params) });
    }
...
function getPagamentos() {
  ...
  return data.slice(1).map(row => {
    ...
    return {
      ...
      camposExtras: (() => { try { return JSON.parse(obj.CamposExtras || '{}'); } catch (_) { return {}; } })()
    };
  });
}
...
function createVendaVendas(params) {
  ...
  // Campos extras cadastrados pela Gerência no editor de formulário (Prompt 1)
  // — qualquer campo do schema que não seja "core" é gravado à parte, em JSON,
  // sem mexer nas colunas fixas usadas por KPIs e relatórios.
  const schema = getFormSchema('vendas');
  const camposExtras = {};
  schema.filter(c => !c.core && c.type !== 'info').forEach(campo => {
    if (params[campo.id] !== undefined && params[campo.id] !== '') camposExtras[campo.label] = params[campo.id];
  });
  ...
  appendMappedRow(sheet, {
    ...
    SolicitadoPor: '', AprovadoPor: '', CamposExtras: JSON.stringify(camposExtras)
  });
...
function requestVendaEdit(params) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const pendente = String(data[i][statusIndex]).trim();
    if (pendente === 'SOLICITACAO DE EDICAO' || pendente === 'SOLICITACAO DE EXCLUSAO') throw new Error('Já existe uma solicitação pendente para este lançamento.');
...
// Perfil Vendedor/Loja (UNIDADE) solicita a EXCLUSÃO de um lançamento já
// feito — mesmo padrão de requestVendaEdit(), só que grava 'SOLICITACAO DE
// EXCLUSAO' em StatusEdicao em vez de 'SOLICITACAO DE EDICAO'.
function requestVendaExclusao(params) {
  const user = requireVendasUser(params, ['UNIDADE']);
  const sheet = getSheetDB();
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const pendente = String(data[i][statusIndex]).trim();
    if (pendente === 'SOLICITACAO DE EDICAO' || pendente === 'SOLICITACAO DE EXCLUSAO') throw new Error('Já existe uma solicitação pendente para este lançamento.');
    sheet.getRange(i + 1, statusIndex + 1).setValue('SOLICITACAO DE EXCLUSAO');
    sheet.getRange(i + 1, reasonIndex + 1).setValue(String(params.motivo || '').trim());
    sheet.getRange(i + 1, requesterIndex + 1).setValue(user.usuario);
    return { id: params.id, statusEdicao: 'SOLICITACAO DE EXCLUSAO' };
  }
  throw new Error('Venda não encontrada.');
}

// Perfil Comercial (GERENCIA) aprova a exclusão. A linha NÃO é apagada da
// planilha (mantém histórico/auditoria) — só recebe Status = 'EXCLUIDO',
// que já sai das pendências e some das listas normais de conferência.
function approveVendaExclusao(params) {
  const user = requireVendasUser(params, ['GERENCIA']);
  const sheet = getSheetDB();
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idIndex = headers.indexOf('ID');
  const mainStatusIndex = headers.indexOf('Status');
  const editStatusIndex = headers.indexOf('StatusEdicao');
  const approvedIndex = headers.indexOf('AprovadoPor');
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idIndex]) !== String(params.id)) continue;
    if (String(data[i][editStatusIndex]).trim() !== 'SOLICITACAO DE EXCLUSAO') throw new Error('Esta venda não possui solicitação de exclusão pendente.');
    sheet.getRange(i + 1, mainStatusIndex + 1).setValue('EXCLUIDO');
    sheet.getRange(i + 1, editStatusIndex + 1).setValue('EXCLUSAO APROVADA');
    sheet.getRange(i + 1, approvedIndex + 1).setValue(user.usuario);
    return { id: params.id, status: 'EXCLUIDO', statusEdicao: 'EXCLUSAO APROVADA' };
  }
  throw new Error('Venda não encontrada.');
}

// Perfil Comercial (GERENCIA) rejeita uma solicitação pendente, seja de
// edição ou de exclusão — o lançamento volta ao normal, sem alterar dados.
function rejectVendaSolicitacao(params) {
  const user = requireVendasUser(params, ['GERENCIA']);
  const sheet = getSheetDB();
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idIndex]) !== String(params.id)) continue;
    const pendente = String(data[i][statusIndex]).trim();
    if (pendente !== 'SOLICITACAO DE EDICAO' && pendente !== 'SOLICITACAO DE EXCLUSAO') throw new Error('Esta venda não possui solicitação pendente.');
    sheet.getRange(i + 1, statusIndex + 1).setValue('SOLICITACAO REJEITADA');
    sheet.getRange(i + 1, approvedIndex + 1).setValue(user.usuario);
    return { id: params.id, statusEdicao: 'SOLICITACAO REJEITADA' };
  }
  throw new Error('Venda não encontrada.');
}
...
// ============================================================
//  5b. GESTÃO DE COLABORADORES (perfil GERENCIA / setor Vendas)
// ============================================================

function getColaboradoresSheet() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(SHEET_COLABORADORES);
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_COLABORADORES);
    sheet.appendRow(['ID', 'Nome', 'Unidade', 'Ativo', 'CriadoPor', 'CriadoEm']);
  }
  return sheet;
}

function getColaboradores() {
  const sheet = getColaboradoresSheet();
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return [];
  const headers = data[0];
  return data.slice(1)
    ...
    .map(row => {
      ...
      return {
        ...
        nome: obj.Nome,
        unidade: obj.Unidade,
        ativo: String(obj.Ativo || 'ATIVO').trim().toUpperCase() !== 'INATIVO',
        criadoPor: obj.CriadoPor || '',
...
// UNIDADE: só enxerga os próprios colaboradores ativos (é o que alimenta o
// dropdown de vendedor do formulário da loja). GERENCIA: enxerga todos, de
// todas as unidades, ativos e inativos, para poder gerenciar.
function listColaboradoresScoped(params) {
  const user = requireVendasUser(params, ['UNIDADE', 'GERENCIA']);
  let rows = getColaboradores();
  if (user.perfil === 'UNIDADE') {
    rows = rows.filter(row => row.ativo && normalizeUnit(row.unidade) === normalizeUnit(user.unidade));
  }
  return rows;
}

function createColaborador(params) {
  const user = requireVendasUser(params, ['GERENCIA']);
  const nome = String(params.nome || '').trim();
  if (!nome) throw new Error('Nome do colaborador é obrigatório.');
  const unidade = normalizeUnit(params.unidade);
  if (!unidade) throw new Error('Unidade é obrigatória.');
  if (UNIDADES_VALIDAS.indexOf(unidade) === -1) throw new Error('Unidade inválida.');

  const sheet = getColaboradoresSheet();
  const id = 'COL' + new Date().getTime();
  sheet.appendRow([id, nome, unidade, 'ATIVO', user.usuario, new Date().toISOString()]);
  return getColaboradores().find(row => row.id === id);
}

function setColaboradorStatus(params) {
  const user = requireVendasUser(params, ['GERENCIA']);
  const novoStatus = String(params.status || '').trim().toUpperCase() === 'INATIVO' ? 'INATIVO' : 'ATIVO';
  const sheet = getColaboradoresSheet();
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idIndex = headers.indexOf('ID');
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idIndex]) === String(params.id)) {
      ...
      return { id: params.id, ativo: novoStatus === 'ATIVO' };
    }
  }
...
// ============================================================
//  5c. FATURAMENTO — venda em lote para revendedores
//      (perfil FATURAMENTO cria; perfil FINANCEIRO confirma o pagamento)
// ============================================================

function getFaturamentoSheet() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(SHEET_FATURAMENTO);
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_FATURAMENTO);
    sheet.appendRow([
      'ID', 'Timestamp', 'Status', 'Revendedor', 'CpfCnpj', 'Cidade',
      'Itens', 'ValorTotal', 'Observacao', 'CriadoPor', 'AtualizadoEm', 'ConfirmadoPor'
    ]);
  }
  return sheet;
}
...
function getFaturamentos() {
  const sheet = getFaturamentoSheet();
  const data = sheet.getDataRange().getValues();
  if (data.length <= 1) return [];
  const headers = data[0];
  return data.slice(1)
    ...
    .map(row => {
      ...
      headers.forEach((h, i) => { obj[h] = row[i]; });
      let itens = [];
      try { itens = JSON.parse(obj.Itens || '[]'); } catch (_) { itens = []; }
      return {
        id: obj.ID,
        timestamp: obj.Timestamp,
        status: obj.Status,
        revendedor: obj.Revendedor,
        cpfCnpj: obj.CpfCnpj,
        cidade: obj.Cidade,
        itens: itens,
        valorTotal: Number(obj.ValorTotal) || 0,
        observacao: obj.Observacao || '',
        criadoPor: obj.CriadoPor || '',
        atualizadoEm: obj.AtualizadoEm || '',
        confirmadoPor: obj.ConfirmadoPor || ''
      };
    });
}

// Só o perfil FATURAMENTO pode lançar — Vendas/Lojas (UNIDADE) e Comercial
// (GERENCIA) não têm essa ação liberada (ver requireVendasUser abaixo).
function createFaturamento(params) {
  const user = requireVendasUser(params, ['FATURAMENTO']);
  const revendedor = String(params.revendedor || '').trim();
  if (!revendedor) throw new Error('Nome do revendedor é obrigatório.');

  let itensBrutos = [];
  try { itensBrutos = typeof params.itens === 'string' ? JSON.parse(params.itens || '[]') : (params.itens || []); }
  catch (_) { throw new Error('Itens inválidos.'); }

  const itens = itensBrutos.map(it => ({
    modelo: String(it.modelo || '').trim(),
    quantidade: Number(it.quantidade) || 0,
    valorUnitario: numberField(it.valorUnitario)
  })).filter(it => it.modelo && it.quantidade > 0);
  if (!itens.length) throw new Error('Informe ao menos um item com modelo e quantidade.');

  const valorTotal = itens.reduce((soma, it) => soma + it.quantidade * it.valorUnitario, 0);
  const sheet = getFaturamentoSheet();
  const id = 'F' + new Date().getTime();
  sheet.appendRow([
    id, new Date().toISOString(), 'PENDENTE DE CONFIRMACAO FINANCEIRA',
    revendedor, String(params.cpfCnpj || '').trim(), String(params.cidade || '').trim(),
    JSON.stringify(itens), valorTotal, String(params.observacao || '').trim(),
    user.usuario, new Date().toISOString(), ''
  ]);
  return getFaturamentos().find(row => row.id === id);
}

// FATURAMENTO enxerga os próprios lançamentos; FINANCEIRO enxerga todos,
// para poder confirmar o pagamento. Nenhum outro perfil acessa esta lista —
// é o que mantém a segregação pedida entre Faturamento e as vendas de loja.
function listFaturamentosScoped(params) {
  requireVendasUser(params, ['FATURAMENTO', 'FINANCEIRO']);
  return getFaturamentos();
}

function confirmFaturamentoPagamento(params) {
  const user = requireVendasUser(params, ['FINANCEIRO']);
  const sheet = getFaturamentoSheet();
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idIndex = headers.indexOf('ID');
  const statusIndex = headers.indexOf('Status');
  const confirmIndex = headers.indexOf('ConfirmadoPor');
  const atualizadoIndex = headers.indexOf('AtualizadoEm');
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idIndex]) !== String(params.id)) continue;
    sheet.getRange(i + 1, statusIndex + 1).setValue('PAGAMENTO CONFIRMADO');
    sheet.getRange(i + 1, confirmIndex + 1).setValue(user.usuario);
    sheet.getRange(i + 1, atualizadoIndex + 1).setValue(new Date().toISOString());
    return { id: params.id, status: 'PAGAMENTO CONFIRMADO' };
  }
  throw new Error('Lançamento de faturamento não encontrado.');
}

// ============================================================
//  5d. FORMULÁRIO DINÂMICO (Prompt 1) — perfil Comercial (GERENCIA)
//      cria/edita/remove campos do formulário de lançamento de Vendas.
//      O esquema fica salvo como JSON na aba FormSchema, e o vendas.html
//      lê esse JSON pra montar o formulário sozinho (formSchemaGet).
// ============================================================

function getFormSchemaSheet() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(SHEET_FORM_SCHEMA);
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_FORM_SCHEMA);
    sheet.appendRow(['FormularioId', 'SchemaJSON']);
    sheet.appendRow(['vendas', JSON.stringify(DEFAULT_VENDAS_SCHEMA)]);
  }
  return sheet;
}

function getFormSchema(formularioId) {
  const id = String(formularioId || 'vendas').trim();
  const sheet = getFormSchemaSheet();
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim() === id) {
      try {
        const campos = JSON.parse(data[i][1] || '[]');
        return campos.sort((a, b) => (a.order || 0) - (b.order || 0));
      } catch (_) { return id === 'vendas' ? DEFAULT_VENDAS_SCHEMA : []; }
    }
  }
  if (id === 'vendas') {
    sheet.appendRow(['vendas', JSON.stringify(DEFAULT_VENDAS_SCHEMA)]);
    return DEFAULT_VENDAS_SCHEMA;
  }
  return [];
}

// Só o perfil Comercial (GERENCIA) grava. Campos "core" não podem ser
// removidos nem trocar de tipo — são os que alimentam KPIs, tabela e
// detalhe do sistema; rótulo, obrigatoriedade e ordem continuam editáveis.
function saveFormSchema(params) {
  requireVendasUser(params, ['GERENCIA']);
  const formularioId = String(params.formularioId || 'vendas').trim();
  let campos;
  try { campos = typeof params.schema === 'string' ? JSON.parse(params.schema) : params.schema; }
  catch (_) { throw new Error('Schema inválido.'); }
  if (!Array.isArray(campos) || !campos.length) throw new Error('Schema inválido.');

  const atuais = getFormSchema(formularioId);
  atuais.filter(c => c.core).forEach(coreField => {
    const enviado = campos.find(c => c.id === coreField.id);
    if (!enviado) throw new Error('O campo "' + coreField.label + '" é obrigatório do sistema e não pode ser removido.');
    if (enviado.type !== coreField.type) throw new Error('O tipo do campo "' + coreField.label + '" não pode ser alterado.');
  });

  campos = campos.map((c, index) => ({ ...c, order: index + 1 }));
  const sheet = getFormSchemaSheet();
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]).trim() === formularioId) {
      sheet.getRange(i + 1, 2).setValue(JSON.stringify(campos));
      return campos;
    }
  }
  sheet.appendRow([formularioId, JSON.stringify(campos)]);
  return campos;
}
